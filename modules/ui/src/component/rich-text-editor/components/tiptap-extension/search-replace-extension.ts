import { Extension, type Dispatch } from '@tiptap/react'
...
declare module '@tiptap/react' {
  interface Commands<ReturnType> {
    search: {
       ...
       */
      replace: (index?: number) => ReturnType
      /**
    ...
    return {
      ...
      replace:
        ...
        ({ editor, state, dispatch }) => {
          ...
          const replaceIndex = index !== undefined ? index : editor.storage.searchAndReplace.resultIndex

          if (!results?.length || replaceIndex < 0 || replaceIndex >= results?.length) {
            return false
          }

          const result = results[replaceIndex]
